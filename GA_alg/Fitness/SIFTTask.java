...
public class SIFTTask implements Runnable {
	...
	//final static private ScriptEngineManager mgr = new ScriptEngineManager();
	//final static private ScriptEngine engine = mgr.getEngineByName("JavaScript");
	...
	static Point[] tmpPoints4Sift_hadar = null;;
	static int tmpPointsSize =0;
	private double[] CalculateFitnessSIFT_hadar(
			InternalMode mode,
			ArrayList<Point> refPoints,
			ArrayList<Point> sensedPoints,
		...
		final double[] defaultFitness4SIFT = new double[]{200000};
		final double[] defaultFitness4SIFT_MOO = new double[]{200000, 200000};
		final double[] defaultFitness4SIFT_MOO2 = new double[]{200000};
		double[] defaultFitness = null;
		if (mode == InternalMode.SIFT)
			defaultFitness = defaultFitness4SIFT.clone();
		else if (mode == InternalMode.SIFT_MOO)
			defaultFitness =  defaultFitness4SIFT_MOO.clone();
		else if (mode == InternalMode.SIFT_MOO2)
			defaultFitness =defaultFitness4SIFT_MOO2.clone();
		...
		final int refPointsSize = refPoints.size(); // hadar: to calculate only once
		final int sensedPointSize = sensedPoints.size();// hadar: to calculate only once
		final int iterations = Math.min(refPointsSize, _maxSIFTMatchesForFitness); // hadar: to calculate only once
		double[] diffX = new double[iterations];
		double[] diffY = new double[iterations];
		double[] diff = new double[iterations];
		int diffArrayRealSize = 0;

		// copy of sensed points
		if (tmpPoints4Sift_hadar == null) {
			tmpPoints4Sift_hadar = new Point[sensedPointSize];
			Point currSensed;
			for (int i = 0; i < sensedPointSize; ++i) {
				 currSensed= sensedPoints.get(i);
				tmpPoints4Sift_hadar[i]=new Point(currSensed.x, currSensed.y);
			}
			tmpPointsSize = sensedPointSize;
		}

		boolean[] used = new boolean[tmpPointsSize];
		...
		for (int i = 0; i < iterations; ++i) {
			int j = i;
			// if there are too many matches - test against random list
			if (refPointsSize > _maxSIFTMatchesForFitness) {
				j = Utils.rand.nextInt(refPointsSize);
			}
			Point refPointJ = refPoints.get(j);
			newPosX = NaN;
			newPosY = NaN;
			AffineTransformation trans = (AffineTransformation) ((GAChromosome)ch)._trans;
			try {
				int tmpIndex = 0;
				if (CONST.GA_SHUFFLE) {
					distance = 10e10;
					for (int k = 0; k <tmpPointsSize; ++k) {
						Point kTmpPoint = tmpPoints4Sift_hadar[k];
						if (!used[k] ) {
							double[] tmpPos = trans.GetValue(kTmpPoint.x, kTmpPoint.y);
							if ( (tmpPos[0]) == POSITIVE_INFINITY || (tmpPos[0]) == NEGATIVE_INFINITY ||
									(tmpPos[0]!=tmpPos[0])  || /* like  Double.isNaN(tmpPos[0])*/
									(tmpPos[1]) == POSITIVE_INFINITY || (tmpPos[1]) == NEGATIVE_INFINITY ||
									(tmpPos[1]!=tmpPos[1]))
								continue;

							double tmpDistance = ((refPointJ.x - tmpPos[0])*(refPointJ.x - tmpPos[0])) + ((refPointJ.y - tmpPos[1])*(refPointJ.y - tmpPos[1]));
					...
					double newPos0 = newPos[0];
					double newPos1 = newPos[1];

					if (!(((newPos0 == POSITIVE_INFINITY) || (newPos0 == NEGATIVE_INFINITY)) || (newPos0 != newPos0) ||
							((newPos1 == POSITIVE_INFINITY) || (newPos1 == NEGATIVE_INFINITY)) || (newPos1 != newPos1))) {
						newPosX = newPos0;
						newPosY = newPos1;
					}
				...
				// hadar: looks like this check happens twice
				if (((newPosX == POSITIVE_INFINITY) || (newPosX == NEGATIVE_INFINITY)) || (newPosX != newPosX) ||
						((newPosY == POSITIVE_INFINITY) || (newPosY == NEGATIVE_INFINITY)) || (newPosY != newPosY))
					continue;

				refX=refPointJ.x;
				refY=refPointJ.y;
//              refs.add(new Point(refX,refY));
//              sensed.add(new Point(sourceX, sourceY));
			...
			double dx = (refX - newPosX)*(refX - newPosX);
			double dy = (refY - newPosY)*(refY - newPosY);

			diffX[diffArrayRealSize] = dx;
			diffY[diffArrayRealSize] = dy;
			diff[diffArrayRealSize] = dx + dy;
			++diffArrayRealSize;
		...
		diff = Arrays.copyOfRange(diff, 0, diffArrayRealSize);
		diffX = Arrays.copyOfRange(diffX, 0, diffArrayRealSize);
		diffY = Arrays.copyOfRange(diffY, 0, diffArrayRealSize);
		Arrays.sort(diff);
		Arrays.sort(diffX);
		Arrays.sort(diffY);
		int num = (diffArrayRealSize * 1) - 1;
		double medianX = diffX[num];
		double medianY = diffY[num];
		double median = diff[num];
	...
	private double[] CalculateFitnessSIFT(
			...
			ArrayList<Point> sensedPoints,
			Chromosome ch) throws MatlabInvocationException, ScriptException {
		if (CONST.PROFILING_MODE ) { //hadar 1302
			return CalculateFitnessSIFT_hadar(mode, refPoints, sensedPoints, ch);
		} else {
			return CalculateFitnessSIFT_orig(mode, refPoints, sensedPoints, ch);
		}
	}

	private double[] CalculateFitnessSIFT_orig(
			InternalMode mode,
			ArrayList<Point> refPoints,
			ArrayList<Point> sensedPoints,
			Chromosome ch) throws MatlabInvocationException, ScriptException {
		...
		int iterations = Math.min(refPoints.size(), _maxSIFTMatchesForFitness); // hadar: to calculate only once
