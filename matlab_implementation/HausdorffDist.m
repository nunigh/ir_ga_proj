...
% hadar : cropping the ref image according to sensed image boundaries
    % need that for HD calculation (so maybe i should limit this part only
    % to HD fitness part
    % this may affect also black pixel which are inside the image and
    % should remain black
    % possible soultion: to transform all the valid blacks (before running imwrap) to non-real
    % color( like 300/-100) and then transform it back 
    % row=1:size(P,1)
    %    for col=1:size(P,2)
    %        if (P(row,col)==0)
    %            Q(row,col)=0;
